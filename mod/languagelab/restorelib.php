<?php //$Id: restorelib.php,v 1.17 2006-09-18 09:13:06 moodler Exp $
    ...
    function languagelab_restore_mods($mod,$restore) {
        ...
        if ($data) {
            ...
            $languagelab->attempts = backup_todb($info['MOD']['#']['VIDEO']['0']['#']);
            $languagelab->attempts = backup_todb($info['MOD']['#']['USE_GRADE_BOOK']['0']['#']);
            $languagelab->attempts = backup_todb($info['MOD']['#']['GROUPTYPE']['0']['#']);
            $languagelab->attempts = backup_todb($info['MOD']['#']['MASTER_TRACK']['0']['#']);
